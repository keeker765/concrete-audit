...
from .yolo_mask import YoloSegMasker


_yolo_masker = None


def _get_yolo_masker():
    global _yolo_masker
    if _yolo_masker is None:
        model_path = Path(__file__).parent.parent / 'daima' / 'best(1).pt'
        _yolo_masker = YoloSegMasker(model_path=model_path, device='cpu')
    return _yolo_masker


def load_and_preprocess(img_path, max_size=1024, roi_mode='dino', rectify=False, use_yolo_mask: bool = True):
    """
    ...
    # YOLO segmentation mask (valid region). Used later to filter keypoints.
    yolo_mask = None
    yolo_box = None
    if use_yolo_mask:
        try:
            yolo_masker = _get_yolo_masker()
            yolo_mask, yolo_box = yolo_masker.segment_main_object(img_bgr)
