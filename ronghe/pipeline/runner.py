...
from .mask_utils import filter_keypoints_by_valid_mask
from .pairing_abc import parse_abc_pairs
from .matching import (
    filter_matches,
    compute_score,
    filter_points_by_mask,
    compute_score_from_points,
    filter_matches_by_center_geometry,
)
...
def parse_sample_pairs(samples_dir):
    """Parse pairs under samples_dir.

    Supports two layouts:
    - ABC layout: samples/<batch>/{A1,A2,...,B1,B2,...}
      (default pairing A<k> <-> B<k>)
    - Legacy layout: *_wet*.jpeg / *_dry*.jpg (QR-based pairing)
    """

    # Prefer ABC layout when there are subdirectories with A/B naming.
    try:
        has_subdirs = any(p.is_dir() for p in samples_dir.iterdir())
    except Exception:
        has_subdirs = False

    if has_subdirs:
        pairs_abc = parse_abc_pairs(samples_dir, left_group='A', right_group='B')
        if pairs_abc:
            return pairs_abc
    ...
    for batch_id, wet_path, dry_path, specimen_no in pairs:
        ...
        try:
            ...
            for rot_k, rot_deg in align_result['candidates']:
                ...
                # YOLO valid mask: keep keypoints on the concrete block (optional)
                vm0 = meta0.get('yolo_mask') if isinstance(meta0, dict) else None
                vm1 = meta1.get('yolo_mask') if isinstance(meta1, dict) else None
                if vm0 is not None:
                    keep0, kpts0_f = filter_keypoints_by_valid_mask(kpts0, vm0)
                else:
                    keep0, kpts0_f = (None, kpts0)
                if vm1 is not None:
                    keep1, kpts1_f = filter_keypoints_by_valid_mask(kpts1, vm1)
                else:
                    keep1, kpts1_f = (None, kpts1)

                # If we filtered keypoints, remap match indices to the filtered index space
                if keep0 is not None or keep1 is not None:
                    # Build old->new index maps
                    import torch
                    if keep0 is None:
                        map0 = None
                    else:
                        map0 = -torch.ones((len(kpts0),), dtype=torch.long)
                        map0[keep0] = torch.arange(len(keep0), dtype=torch.long)
                    if keep1 is None:
                        map1 = None
                    else:
                        map1 = -torch.ones((len(kpts1),), dtype=torch.long)
                        map1[keep1] = torch.arange(len(keep1), dtype=torch.long)
                    ...
                    if map1 is not None:
                        ...
                        mr1 = mr[:,1]
                    ok = (mr0 >= 0) & (mr1 >= 0)
                    matches_raw = torch.stack([mr0[ok], mr1[ok]], dim=1)
                    if scores_raw is not None and len(scores_raw) == len(mr):
                        scores_raw = scores_raw[ok]
                    kpts0, kpts1 = kpts0_f, kpts1_f

                matches_f, scores_f = filter_matches(
                    kpts0, kpts1, matches_raw, scores_raw, m_r, mask_dry_a)

                # Geometry filter in sticker-centered coordinate system (cosine + euclidean)
                matches_f, scores_f, geo_stats = filter_matches_by_center_geometry(
                    kpts0, kpts1, matches_f, scores_f,
                    center0=(center0[0], center0[1]) if center0 is not None else None,
                    center1=(center1[0], center1[1]) if center1 is not None else None,
                )
            ...
            print(f"  Raw: {best['n_raw']} -> Filtered: {n_matches} -> "
                  f"Inliers: {n_inliers}")
            if 'geo' in best and best['geo'] is not None:
                gs = best['geo']
                if not gs.get('geo_skipped', False):
                    print(
                        f"  Geo: kept={gs.get('geo_kept', 0)}/{gs.get('geo_total', 0)} "
                        f"(ratio={gs.get('geo_keep_ratio', 0):.2f}) | "
                        f"mean_cos={gs.get('geo_mean_cos', 0):.3f} | "
                        f"mean_dr={gs.get('geo_mean_dr', 0):.1f}"
                    )
